...
export function useDataTablePinnedColumns<T>({
  ...
  const recalculate = useCallback(() => {
    ...
    for (const col of columns) {
      ...
      if (col.pinned) {
        ...
        pinLastColumn &&
        acc === renderedAccessors[renderedAccessors.length - 1] &&
        !resolvedPinned.has(acc)
      ) {
  ...
  useEffect(() => {
    ...
    const isTheadSource = !!theadRef.current;
    const row = isTheadSource
      ? section.querySelector('tr:last-of-type')
      : section.querySelector('tr:first-of-type');
    if (!row) return;
